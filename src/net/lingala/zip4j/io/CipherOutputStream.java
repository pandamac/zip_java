...
public class CipherOutputStream extends BaseOutputStream {
		...
		try {
			...
			if (this.outputStream instanceof net.lingala.zip4j.io.SplitOutputStream) {
				if (totalBytesWritten == 4) {
					...
					fileHeader.setOffsetLocalHeader(((net.lingala.zip4j.io.SplitOutputStream)outputStream).getFilePointer());
				}
			} else {
				if (totalBytesWritten == 4) {
					fileHeader.setOffsetLocalHeader(4);
	...
	private void initZipModel(ZipModel zipModel) {
		...
		if (this.outputStream instanceof net.lingala.zip4j.io.SplitOutputStream) {
			if (((net.lingala.zip4j.io.SplitOutputStream)outputStream).isSplitZipFile()) {
				this.zipModel.setSplitArchive(true);
				this.zipModel.setSplitLength(((net.lingala.zip4j.io.SplitOutputStream)outputStream).getSplitLength());
			}
		}
...
//					sourceFile.getAbsolutePath(), zipParameters.getRootFolderInZip(), zipParameters.getDefaultFolderPath());
			fileName = zipParameters.getRootFolderInZip();//by panda
			System.out.println("copy to apk         ==> "+fileName);
